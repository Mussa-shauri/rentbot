...
export default function Login({ onLogin, onGoRegister }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (data.token) {
        localStorage.setItem('rentbot_token', data.token);
        localStorage.setItem('rentbot_session', JSON.stringify(data.user));
        onLogin(data.user);
      } else {
...
const styles = {
  ...
  card:      { background: '#fff', borderRadius: 16, padding: '36px 32px', width: '100%', maxWidth: 420, boxShadow: '0 4px 32px rgba(0,0,0,0.10)', display: 'flex', flexDirection: 'column', gap: 14 },
